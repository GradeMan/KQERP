...
namespace ERP.Service
{
    ...
    public class ProductProdInfoService : IProductProdInfoService
	{
        ...
        {
        	ERP_M001_Product_ProdInfo.ModifyDt = DateTime.Now;
            tb_Sys_User tempUser = appCacheService.GetItem("user") as tb_Sys_User;
            ERP_M001_Product_ProdInfo.ModifyUser = tempUser.UserName;
            ERP_M001_Product_ProdInfo.Validate();
        ...
        {
            var q = this.ERP_M001_Product_ProdInfoRepository.GetMany(i => i.ItemCode == itemCode && i.ProcessCode == processCode).Select(i => i.ProjCode+":"+i.ProContent).ToList();
            return q.ToArray();
        }
    }
}